...
@router.get("/subscription/{subscription_uuid}", response_model=schemas.SubscriptionOutSchema)
def get_subscription_for_id(subscription_uuid: str,
...
def create_new_subscription(subscription: schemas.SubscriptionInSchema,
                            session: Session = Depends(get_session)):
    """Create a new subscription"""
    return services.create_subscription(session, subscription)


@router.delete("/subscription/{subscription_uuid}", response_class=Response,
               status_code=status.HTTP_204_NO_CONTENT)
def delete_subscription(subscription_uuid: str, session: Session = Depends(get_session)):
